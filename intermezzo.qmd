...
With a *senso strictu* survivorship analysis where the "event" is death, the event only occurs once.
...
As such the primary distinction I make here is the same distinction as that made in @tanimura2021sql.
...
One example that is given does not help much: "if we are concerned about the number of students who are still in school after a certain number of years, we could look for the absence of a graduation event in a data set" [p. 154].
