...
### Censuses: Events that can happen more than once

In many contexts, the event of interest will be a once-only event.
In some cases, the one-off nature of the event will be a function of the underlying realities (e.g., *senso strictu* survivorship analysis in which individuals only die ones).
...
A subscription streaming service such as Netflix or Max might have customers who sign up for a monthly subscription, drop off some months later, then rejoin at a later date.
...
### Stints: Events that can happen more than once
