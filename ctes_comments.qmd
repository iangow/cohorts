# Cohort Analysis -- Comments and CTEs
...
cohorts AS (
  SELECT date_part('year', age(term_start, first_term)) as period,
...
cohorts AS (
  SELECT id_bioguide, first_term,
    ...
    date_part('year', age(date, first_term)) AS period
  FROM first_terms
  JOIN legislators_terms 
  USING (id_bioguide)
  LEFT JOIN year_ends
...
cohorts AS (
  SELECT a.term_type,
    coalesce(year(age(c.date,a.first_term)), 0) AS period,
    count(DISTINCT a.id_bioguide) AS cohort_retained
  FROM first_terms a
  JOIN legislators_terms b
  ON a.id_bioguide = b.id_bioguide and b.term_start >= a.min_start
  LEFT JOIN year_ends c 
  ON c.date BETWEEN b.term_start AND b.term_end AND c.year >= 2000
  GROUP BY 1,2)
...
rep_sen_centuries AS (
  ...
  USING (id_bioguide)
  WHERE c.term_type = 'sen' AND c.term_start > b.first_term
  GROUP BY 1)
...
SELECT date_part('century', first_term) AS century,
  first_type,
...
SELECT date, century,
  ...
  legislators / sum(legislators) over (partition by date) as pct_century
FROM cohorts a
ORDER BY 1 DESC
```
