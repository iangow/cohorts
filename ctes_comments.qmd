...
Third, as a concomitant of using the CTEs, I clean up the SQL in other ways.
  ...
  - I can't take the "leading commas" used in @tanimura2021sql: ["We're living in a society!"](@tanimura2021sql) ... so these go!
  - I make other changes to the SQL for a more consistent style (e.g., SQL keywords are capitalized).
  - I use `USING` for cleaner SQL. For example `USING (id_bioguide)` is shorter than `ON a.id_bioguide = b.id_bioguide` and often obviates the need to use aliases (`a` and `b` here).
  
Fourth, I delete some text, such as the introductory text that was in this very spot in the original chapter.
Fifth, I modify some of the original text when it no longer reflects the approach used here.
:::
